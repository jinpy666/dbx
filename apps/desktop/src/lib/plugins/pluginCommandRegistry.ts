...
export interface PluginSidebarCommandEntry {
  pluginId: string;
  pluginName: string;
  commandId: string;
  label: string;
...
export function usePluginSidebarCommands() {
  const { locale } = useI18n();
  const entries = ref<PluginSidebarCommandEntry[]>([]);
  ...
  async function refresh() {
    try {
      ...
      entries.value = nextRegistry.listSidebarMenuCommands().map(({ plugin, command }: { plugin: { manifest: { id: string; name: string } }; command: PluginCommandContribution }) => ({
      ...
      console.warn("[DBX][plugin:sidebar-entries]", cause);
      entries.value = [];
      registry.value = null;
    }
  }

  function open(entry: PluginSidebarCommandEntry): PluginCommandExecutionResult {
